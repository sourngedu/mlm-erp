...
// save and load the fields in and/or from localStorage
    ...
    var $customer = $('#prcustomer');
    $customer.change(function (e) {
        localStorage.setItem('prcustomer', $(this).val());
    });
    if (prcustomer = localStorage.getItem('prcustomer')) {
        $customer.val(prcustomer).select2({
            minimumInputLength: 1,
            data: [],
            initSelection: function (element, callback) {
                $.ajax({
...
$(document).on('click', '.qudel', function () {
     ...
     * Show manual item addition modal
     ...
     $(document).on('click', '#addManually', function (e) {
        if (count == 1) {
            pritems = {};
            if ($('#prwarehouse').val() && $('#prcustomer').val()) {
                $('#prcustomer').select2("readonly", true);
    ...
    $(document).on("click", '#removeReadonly', function () {
       $('#prcustomer').select2('readonly', false);
       //$('#prwarehouse').select2('readonly', false);
       return false;
    });
