    ...
    $('#reset').click(function (e) {
            bootbox.confirm(lang.r_u_sure, function (result) {
                if (result) {
                    ...
                    if (localStorage.getItem('porcustomer')) {
                        localStorage.removeItem('porcustomer');
                    }
...
// save and load the fields in and/or from localStorage
    ...
    var $customer = $('#porcustomer');
    $customer.change(function (e) {
        localStorage.setItem('porcustomer', $(this).val());
    });
    if (porcustomer = localStorage.getItem('porcustomer')) {
        $customer.val(porcustomer).select2({
            minimumInputLength: 1,
            data: [],
            initSelection: function (element, callback) {
                $.ajax({
...
$(document).on('click', '.qudel', function () {
     ...
     * Show manual item addition modal
     ...
     $(document).on('click', '#addManually', function (e) {
        if (count == 1) {
            poritems = {};
            if ($('#porwarehouse').val() && $('#porcustomer').val()) {
                $('#porcustomer').select2("readonly", true);
    ...
    $(document).on("click", '#removeReadonly', function () {
       $('#porcustomer').select2('readonly', false);
       //$('#porwarehouse').select2('readonly', false);
       return false;
    });
...
function loadItems() {
    ...
    if (localStorage.getItem('poritems')) {
        ...
        $.each(sortedItems, function () {
            ...
            tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + formatDecimal(item_price) + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="pnote" name="pnote[]" type="hidden" value="' + pnote + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(item_price) + '</span></td>';
