    ...
    if (localStorage.getItem('reitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            if (ds.indexOf("%") !== -1) {
                var pds = ds.split("%");
                if (!isNaN(pds[0])) {
                    item_discount = formatDecimal((((unit_cost) * parseFloat(pds[0])) / 100), 4);
                } else {
			...
			if(item_discount>0){
				var item_discount_percent = '('+formatDecimal((item_discount * 100)/unit_cost)+'%)';
			}else{
			...
			tr_html += '<input class="form-control input-sm text-right rcost" name="net_cost[]" type="hidden" id="cost_' + row_no + '" value="' + formatDecimal(item_cost) + '"><input class="rucost" name="unit_cost[]" type="hidden" value="' + unit_cost + '"><input class="realucost" name="real_unit_cost[]" type="hidden" value="' + item.row.real_unit_cost + '">';
			...
			if (site.settings.product_serial == 1) {
				tr_html += '<input class="rserial" name="serial_no[]" type="hidden" value="'+item.row.serial_no+'" id="serial_no_' + row_no + '"/>';
            }
			tr_html += '<td class="text-center"><span class="received">' + formatDecimal(item_received) + '</span></td>';
            ...
            total += formatDecimal(((parseFloat(item_cost) + parseFloat(pr_tax_val)) * parseFloat(item_received)), 4);
