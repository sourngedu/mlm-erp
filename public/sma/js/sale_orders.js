...
$(document).on('click', '.qudel', function () {
	...
	$(document).on('change', '#pgroups', function () {
		...
		$('#punit').select2('val', soitems[item_id].row.base_unit);
     ...
     * Edit Row Method
     ...
     $(document).on('click', '#editItem', function () {
		...
		if (site.settings.price_groups == 1) {
			soitems[item_id].row.price_group = $('#pprice').val();
			soitems[item_id].row.price_group_id = $('#pgroup').val();
		}
        ...
        soitems[item_id].row.real_unit_price = price,
	...
	$(document).on("change", '.slength, .swidth, .square', function () {
		...
		item_id = row.attr('data-item-id');
		soitems[item_id].row.slength = slength;
